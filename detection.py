...
def calculate_threshold_for_specificity(test_losses, desired_specificity):
    ...
    index = int((1 - desired_specificity) * len(sorted_losses))
...
color = 'color'
i = 1 # pick a name out of the model name list
epoch=100
lr=0.001

for desired_precision in [.9,.8,.7,.6,.5]:

    model_name = [f'UNet_2D_newtrainset_2Layer_{color}_{dropout}dropout_{channels}channels_epoch{epoch}_lr0.001',
                  f'UNet_2D_newtrainset_2Layer_{color}_{dropout}dropout_{channels}channels_epoch{epoch}_lr{lr}finetuned',
    ...
    for subdir in os.listdir(real_world_testpath):
        # Check if subdir name starts with 'realworld' (case-insensitive)
        if subdir.lower().startswith('realworld'):
            ...
            if os.path.isdir(full_subdir_path):
                ...
                if desired_precision != 0:
                    optimal_threshold = calculate_threshold_for_specificity(combined_losses, combined_labels, desired_precision)
